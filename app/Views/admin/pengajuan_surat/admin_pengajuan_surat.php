<main class="content">
        ...
        <div class="container-fluid px-0">
            <div class="d-flex justify-content-between w-100">
                <div class="d-flex align-items-center w-100">
                    <form class="navbar-search form-inline" method="GET"
                        action="<?= base_url('admin/pengajuan_surat') ?>">
                        <div class="input-group input-group-merge search-bar">
                            ...
                            <input type="text" name="keyword" class="form-control" placeholder="Cari nama, no HP,..."
                                value="<?= esc($keyword ?? '') ?>">
                        </div>
                    </form>
                </div>
    ...
    <div class="py-4">
        ...
        <div class="d-flex justify-content-between w-100 flex-wrap">
            ...
            <div>
                <a href="<?= base_url('admin/pengajuan_surat/export_excel') ?>"
                            ...
                            <tr id="row-<?= $row['id_pengajuan_surat'] ?>">
                                ...
                                <td>
                                    <?php if ($row['file_surat']): ?>
                                        <a href="<?= base_url('uploads/surat/' . $row['file_surat']) ?>" target="_blank">Lihat
                                            File</a>
                                    <?php else: ?>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-success"
                                        onclick="konfirmasiStatus(<?= $row['id_pengajuan_surat'] ?>, 'Disetujui')">Setujui</button>
                                    <button class="btn btn-sm btn-danger"
                                        onclick="konfirmasiStatus(<?= $row['id_pengajuan_surat'] ?>, 'Ditolak')">Tolak</button>
                                    <button class="btn btn-sm btn-warning"
                                        onclick="konfirmasiStatus(<?= $row['id_pengajuan_surat'] ?>, 'Pending')">Pending</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
